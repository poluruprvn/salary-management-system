...
- Per point: `date`, `headcount`, `salaried`, `gross_cents`, `loaded_cents`. The same run rate columns and the same lateral, read at each point's date instead of one `as_of`. One query, not thirteen requests.
- The dates are computed in Ruby, `(as_of << n).end_of_month`, and passed to `unnest`. `generate_series` over `date` returns `timestamptz`, which depends on the session time zone, and month-end arithmetic in SQL is harder to read than in Ruby. `Date#<<` clamps, so February ends on the 28th or 29th.
- Per interval, attached to the point that closes it: `hires` dated inside it, `exits` whose last paid day is on or after the previous point and before this one, and `raises` and `raise_delta_cents` from live revisions effective inside it that have a previous live revision, which excludes starting salaries. The exit date is paid, so an exit on a point's date is still in that point's headcount: counting it at the next point keeps each point's headcount equal to the previous one plus hires less exits. The first point closes no interval, so its four are null. The delta comes from `SalaryRevision::PREVIOUS_AMOUNT`, the `LAG` `history_for` uses. A filter chained onto the window hides rows from `LAG`, so the interval date filter is a join on the window's output. It is a plain join, not a lateral: Rails' `with` has no `MATERIALIZED`, and a lateral re-ran the window once per point, which cost 240 ms. A backdated void changes these numbers, which is correct: they are a re-read of the dates, like everything else here.
